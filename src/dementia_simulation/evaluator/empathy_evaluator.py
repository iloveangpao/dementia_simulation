...
class EmpathyEvaluator:
    ...
    def evaluate_response_patterns(self, response: str) -> EmpathyMetrics:
        ...
        metrics.non_confrontational_score = max(
            0.0, min(1.0, non_confrontational_score)
        )
    ...
    def analyze_conversation_flow(
        ...
        """
        caregiver_messages = [
            msg for msg in conversation_history if msg.get("speaker") == "caregiver"
        ]
        ...
        if len(empathy_scores) > 1:
            variance = sum(
                (x - sum(empathy_scores) / len(empathy_scores)) ** 2
                for x in empathy_scores
            ) / len(empathy_scores)
        ...
        if len(patient_moods) > 1:
            mood_changes = sum(
                1
                for i in range(1, len(patient_moods))
                if patient_moods[i] != patient_moods[i - 1]
            )
            if mood_changes > 0:
                # Check if caregiver responses adapted to mood changes
                adaptability = min(
                    1.0, len(caregiver_messages) / (mood_changes + 1) * 0.5
                )
        ...
        for msg in caregiver_messages:
            ...
            if any(
                ...
                for word in ["tell me", "what do you", "how do you"]
            ):
                engagement_indicators += 1
    ...
    def generate_feedback(
        ...
        if metrics.validation_score >= 0.7:
            strengths.append(
                "Excellent at validating patient feelings without correction"
            )
        ...
        if metrics.emotional_support_score >= 0.7:
            ...
            feedback.append(
                "Use comforting phrases and acknowledge the patient's emotional state"
            )

        # Patience feedback
        if metrics.patience_score >= 0.7:
            strengths.append(
                "Demonstrates excellent patience with repetition and confusion"
            )
        ...
        if metrics.communication_clarity_score >= 0.7:
            strengths.append(
                "Uses clear, simple communication appropriate for dementia care"
            )
            ...
            feedback.append(
                "Break complex ideas into smaller, easier-to-understand parts"
            )
        ...
        if conversation_metrics["consistency"] >= 0.7:
            ...
            improvements.append(
                "Work on maintaining consistent empathy levels throughout interactions"
            )
        ...
        if conversation_metrics["engagement"] >= 0.7:
            ...
            improvements.append(
                "Ask more open-ended questions to encourage patient engagement"
            )

        return feedback, strengths, improvements

    async def evaluate_conversation(
        self, conversation_history: List[Dict[str, Any]], caregiver_responses: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        if all_metrics:
            avg_metrics.validation_score = sum(
                m.validation_score for m in all_metrics
            ) / len(all_metrics)
            avg_metrics.emotional_support_score = sum(
                m.emotional_support_score for m in all_metrics
            ) / len(all_metrics)
            avg_metrics.respect_score = sum(m.respect_score for m in all_metrics) / len(
                all_metrics
            )
            avg_metrics.patience_score = sum(
                m.patience_score for m in all_metrics
            ) / len(all_metrics)
            avg_metrics.communication_clarity_score = sum(
                m.communication_clarity_score for m in all_metrics
            ) / len(all_metrics)
            avg_metrics.non_confrontational_score = sum(
                m.non_confrontational_score for m in all_metrics
            ) / len(all_metrics)
        ...
        feedback, strengths, improvements = self.generate_feedback(
            avg_metrics, conversation_metrics
        )
...
if __name__ == "__main__":
    ...
    test_conversation = [
        {
            "speaker": "caregiver",
            "message": "I understand this must be confusing for you.",
        },
        {"speaker": "patient", "message": "Where am I?", "mood": "confused"},
        {
            "speaker": "caregiver",
            "message": "You're in a safe place. I'm here with you.",
        },
    ...
    async def test_evaluation():
        result = await evaluator.evaluate_conversation(
            test_conversation, test_responses
        )
