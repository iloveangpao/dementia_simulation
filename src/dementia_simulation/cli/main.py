...
@cli.command()
@click.option('--persona-id', help='Persona ID to use (persona_1, persona_2, persona_3)')
...
def chat(persona_id: Optional[str], save_session: bool):
    ...
    async def run_chat():
        ...
        # Select persona
        if persona_id and persona_id in cli_session.personas:
            persona = cli_session.personas[persona_id]
        else:
            click.echo("Available personas:")
            for pid, persona in cli_session.personas.items():
                click.echo(f"  {pid}: {persona.name} - {persona.age} years old, {persona.stage.value} dementia")
        ...
        while True:
            try:
                ...
                click.echo(f"   💭 Mood: {response.persona_mood} | Confidence: {response.confidence_score:.2f}")
...
def analyze(session_file: str):
    ...
    async def analyze_session():
        ...
        try:
            ...
            caregiver_responses = [
                msg['message'] for msg in session_data['conversation_history']
...
def server(port: int, host: str):
    ...
    try:
        ...
        click.echo("❌ FastAPI/Uvicorn not available. Install with: pip install fastapi uvicorn")
    except Exception as e:
