...
class PersonaState:
    ...
    preferred_topics: List[str] = field(
        default_factory=lambda: ["family", "past", "home"]
    )
...
class DementiaSimulationRules:
    ...
    def apply_forgetting_rules(persona: PersonaState, new_input: str) -> PersonaState:
        ...
        persona.confusion_level = min(
            1.0, persona.confusion_level + random.uniform(0.0, 0.1)
        )

        return persona

    @staticmethod
    def apply_mood_rules(
        persona: PersonaState, conversation_context: List[str]
    ) -> PersonaState:
...
def rag_pipeline(query: str, context: List[str], persona_state: PersonaState) -> str:
    ...
    In a real implementation, this would integrate with a language model and
    knowledge base. For now, this provides rule-based responses that simulate
    dementia patterns.
    """
...
class DementiaSimulationChat:
    ...
    def chat_loop(self, user_input: str) -> Tuple[str, Dict[str, Any]]:
        """
        Main chat loop that processes user input and returns patient response
        with persona state.
        ...
        self.persona_state = self.rules.apply_forgetting_rules(
            self.persona_state, user_input
        )
        ...
        # Apply mood rules
        self.persona_state = self.rules.apply_mood_rules(
            self.persona_state, recent_messages
        )
...
def run_demo_conversation():
    ...
    for user_input in user_inputs:
        ...
        print(
            f"[Mood: {persona_state['current_mood']}, "
